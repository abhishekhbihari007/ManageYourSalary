...
export default function InHandSalaryCalculator() {
  ...
  const calculateSalary = (regimeOverride?: "old" | "new") => {
    try {
      const currentRegime = regimeOverride || taxRegime;
      console.log("calculateSalary called", { ctc, variablePay, taxRegime: currentRegime, pfOption });
    ...
    // ET Money: Special = CTC - Basic - HRA - Employee PF - Gratuity - Employer PF - Variable Pay
    const specialAllowance = Math.max(0, ctcValue - basicSalary - hra - pfEmployee - gratuity - pfEmployer - variablePayValue);
    ...
    // Step 8: Calculate Gross Salary = Basic + HRA + Special Allowance + Variable Pay (ET Money method)
    // This is the correct Gross Salary for tax calculation - Variable Pay must be included
    const grossSalary = basicSalary + hra + specialAllowanceYearly + variablePayValue;
    ...
    if (currentRegime === "new") {
      ...
      // Taxable Income = Yearly Gross Salary - Standard Deduction ONLY
      // New Regime does NOT subtract Employee PF for tax calculation (Employee PF is NOT a deduction)
      ...
      // Priority: 1. Manual annualHRA input, 2. Calculate from monthlyRent, 3. Calculate maximum (if exemptions enabled)
      if (annualHRA && parseFloat(annualHRA) > 0) {
        // Use manual HRA exemption input
        hraExemption = Math.min(parseFloat(annualHRA), hra);
      } else if (monthlyRent && parseFloat(monthlyRent) > 0) {
        // Calculate from rent paid
        const annualRent = parseFloat(monthlyRent) * 12;
        const hraPercentage = hraOption === "50" ? 0.5 : 0.4;
        // HRA exemption is minimum of:
        // 1. Actual HRA received
        // 2. Rent paid - 10% of basic salary
        // 3. 50% of basic (metro) or 40% of basic (non-metro)
        const hraExemption1 = hra;
        const hraExemption2 = Math.max(0, annualRent - (basicSalary * 0.1));
        const hraExemption3 = basicSalary * hraPercentage;
        hraExemption = Math.min(hraExemption1, hraExemption2, hraExemption3);
      } else if (exemptionsEnabled) {
        // Calculate maximum HRA exemption (assumed max) when exemptions are enabled
        // Maximum is the minimum of: Actual HRA received, 50% of basic (metro) or 40% of basic (non-metro)
        const hraPercentage = hraOption === "50" ? 0.5 : 0.4;
        hraExemption = Math.min(hra, basicSalary * hraPercentage);
      } else {
        hraExemption = 0;
      }
      
      // Health Insurance (80D) should be independent - always available if exemptions enabled
      if (exemptionsEnabled) {
        section80D = ageValue < 60 ? 25000 : 50000;
      }
      
      // Section 80C: Employee PF is part of Section 80C, so it should be included within the ₹1,50,000 limit
      if (exemptionsEnabled) {
        const section80CInputValue = parseFloat(section80CInput) || 0;
        // Employee PF is already part of 80C, so we need to calculate remaining 80C limit
        // Total 80C = Employee PF + Other 80C investments (PPF, ELSS, etc.)
        // Maximum total 80C = ₹1,50,000
        const remaining80CLimit = Math.max(0, 150000 - pfEmployee);
        if (section80CInputValue > 0) {
          // Other 80C investments (PPF, ELSS, etc.) - capped at remaining limit
          const other80C = Math.min(remaining80CLimit, section80CInputValue);
          // Total 80C = Employee PF + Other 80C investments (capped at ₹1,50,000)
          section80C = Math.min(150000, pfEmployee + other80C);
        } else {
          // If no other investments, only Employee PF counts (up to ₹1,50,000)
          section80C = Math.min(150000, pfEmployee);
        }
      }
      
      // Taxable Income = Yearly Gross Salary - Standard Deduction - Employee PF (included in 80C) - HRA Exemption - Section 80C (total including PF) - Section 80D
      // Note: Employee PF is NOT subtracted separately since it's part of Section 80C
      taxableIncome = Math.max(0, grossSalary - standardDeduction - (exemptionsEnabled ? hraExemption : 0) - section80C - section80D);
    ...
    const taxSavings = exemptionsEnabled && currentRegime === "old" ? taxWithoutExemptions - yearlyTax : 0;
  ...
  }, [taxRegime, exemptionsEnabled, section80CInput, age, monthlyRent, hraOption]);
                ...
                <div className="mb-4 w-full">
                  <Tabs 
                    ...
                    onValueChange={(value) => {
                      ...
                      // Immediately recalculate if we have results, passing the new regime directly
                      if (hasCalculatedRef.current && ctc && parseFloat(ctc) > 0) {
                        calculateSalary(newRegime);
                      }
                    }} 
                    className="w-full"
                  >
                  ...
                  <CardContent className="pt-6">
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-5xl font-bold text-primary">
                        {(() => {
                          const monthlySalary = result.netSalary / 12;
                          const yearlySalary = result.netSalary;
                          
                          if (viewMode === "monthly") {
                            // Show in thousands (K) when below 1 lakh, otherwise show in lakhs
                            if (monthlySalary < 100000) {
                              return `₹${(monthlySalary / 1000).toFixed(1)}K /month`;
                            } else {
                              return `₹${(monthlySalary / 100000).toFixed(2)} Lacs /month`;
                            }
                          } else {
                            // For yearly, show in lakhs if >= 1 lakh, otherwise show in thousands
                            if (yearlySalary < 100000) {
                              return `₹${(yearlySalary / 1000).toFixed(1)}K /year`;
                            } else {
                              return `₹${(yearlySalary / 100000).toFixed(2)} Lacs /year`;
                            }
                          }
                        })()}
                      </p>
                      ...
                      <>
                        <div className="flex justify-between text-destructive">
                          <span className="text-sm">
                            Standard Deduction
                            <span className="text-xs text-muted-foreground ml-1">(applied by default)</span>
                          </span>
                        ...
                        {taxRegime === "old" && exemptionsEnabled && (
                          <>
                            <div className="flex justify-between text-destructive">
                              <span className="text-sm">80C (Employee PF + Other investments)</span>
                    ...
                    <div className="space-y-2">
                      ...
                      {result.variablePay > 0 && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold">{formatCurrency(viewMode === "monthly" ? result.variablePay / 12 : result.variablePay)}</span>
                        </div>
                      )}
                  ...
                  {hraExpanded && (
                    <CardContent>
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <Label>Annual HRA Exemption (Optional - for manual override)</Label>
                          <div className="flex items-center gap-2">
                            <Input 
                              type="text" 
                              placeholder="Auto-calculated if rent provided"
                              value={annualHRA ? formatCurrency(parseFloat(annualHRA) || 0).replace('₹', '').trim() : ""} 
                              onChange={(e) => {
                                const value = e.target.value.replace(/[₹,\s]/g, '');
                          ...
                          <p className="text-xs text-muted-foreground">
                            {result && exemptionsEnabled && result.hraExemption > 0
                              ? `Current exemption: ${formatCurrency(result.hraExemption)} (calculated automatically)`
                              : "Leave empty to auto-calculate based on rent or use maximum exemption when exemptions enabled"}
                          </p>
                        </div>
                      </div>
                    </CardContent>
